...
### 4. Bundle identifier
- `CFBundleIdentifier` = `com.giraybatiturk.forcequitx` (or whatever's currently in `PRODUCT_BUNDLE_IDENTIFIER`). Must match what notarization and SMAppService expect — never change it across releases or existing users lose state.
